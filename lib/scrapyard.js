...
var scrapyard = function(config) {
	...
	s.queue = async.queue(function(task, callback){
		...
		s.perform(task.url, task.type, task.encoding, function(err,data){
	...
	s.scrape = function(url, type, encoding, callback, it) {
		...
		if (_it > config.retry) {
			...
			s.get(url, type, encoding, function(err, data){
				if (err) {
	...
	s.get = function(url, type, encoding, callback) {
		s.queue.push({'url': url, 'type': type, 'encoding': encoding}, function(err, data){
			callback(err, data);
		});
	};

	/* perform */
	s.perform = function(url, type, encoding, callback) {
		switch (type) {
			case 'xml': s.xml(url, encoding, callback); break;
			case 'json': s.json(url, encoding, callback); break;
			case 'html': s.html(url, encoding, callback); break;
			default: callback(new Error('invalid type '+type));
		}
	}
	
	/* get an xml resource */
	s.xml = function(url, encoding, callback) {
		s.fetch(url, encoding, function(err, data){
			if (err) {
				callback(err);
			} else {
	...
	/* get a json resource */
	s.json = function(url, encoding, callback) {
		s.fetch(url, encoding, function(err,data){
			if (err) {
				callback(err);
			} else {
	...
	s.html = function(url, encoding, callback) {
		s.fetch(url, encoding, function(err,data){
			if (err) {
				callback(err);
			} else {
	...
	s.fetch = function(url, encoding, callback) {
		s.storage.valid(url, s.config.timeout, function(valid){
			if (valid) {
				...
				s.storage.get(url, function(err, data){
					if (err) {
						/* get and save */
						request.get({url: url, encoding: encoding}, function(err, response, data){
				...
				/* get a new one */
				request({url: url, encoding: encoding}, function(err, response, data){
					if (err || response.statusCode !== 200) {
						if (s.config.debug === true) console.error('[err!]'.magenta.inverse.bold, url.yellow);
						callback(err||new Error(response.statusCode));
					} else {
